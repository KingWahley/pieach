...
// Stylized vector silhouette avatar for detail page (large scale)
function DetailAvatar({ gender }) {
  return (
    <svg 
      className="w-full h-full text-neutral-500/25" 
      viewBox="0 0 100 120" 
      fill="none"
    >
      <defs>
        <radialGradient id="detailGrad" cx="50%" cy="50%" r="50%" fx="50%" fy="50%">
          <stop offset="0%" stopColor="#ffffff" stopOpacity="0.9" />
          <stop offset="100%" stopColor="#c5a880" stopOpacity="0.1" />
        </radialGradient>
      </defs>
      <rect width="100" height="120" fill="url(#detailGrad)" />
      <circle cx="50" cy="40" r="18" className="fill-neutral-600/20 stroke-neutral-600/10" strokeWidth="0.5" />
      {gender === "female" ? (
        <path 
          d="M29 38 C29 16, 71 16, 71 38 C71 55, 64 60, 64 62 C60 48, 40 48, 36 62 C36 60, 29 55, 29 38 Z" 
          className="fill-neutral-700/30" 
        />
      ) : (
        <path 
          d="M32 36 C32 22, 68 22, 68 36 C68 30, 32 30, 32 36 Z" 
          className="fill-neutral-800/25" 
        />
      )}
      <path 
        d="M15 108 C15 78, 25 72, 50 72 C75 72, 85 78, 85 108 Z" 
        className="fill-neutral-600/25 stroke-neutral-600/10" 
        strokeWidth="0.5" 
      />
      <path d="M38 72 L50 84 L62 72" className="stroke-neutral-400/80" strokeWidth="1" />
    </svg>
  );
}
...
export default async function TeamMemberDetailsPage({ params }) {
  ...
  return (
    <div className="bg-white text-neutral-900 pt-32">
      ...
      <OurTeamSection excludeSlug={member.slug} title="Other Team Members" />
