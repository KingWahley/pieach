...
export function createStore(initialData, idPrefix = 'id', storeName) {
  ...
  const api = {
    ...
    createItem: (item) => {
      ...
      // Convert to DB format first to compute fields (like gallery, etc.)
      const dbItem = mapJsToDb(newItem, storeName);
      // Map back to JS to get computed/normalized fields in store
      const jsNewItem = mapDbToJs(dbItem, storeName);

      // Optimistic update
      store = [jsNewItem, ...store];
      api.notify();
    ...
    updateItem: (id, updates) => {
      ...
      const dbUpdates = mapJsToDb(mergedItem, storeName);
      
      // Map back to JS to get computed/normalized fields in store
      const jsUpdatedItem = mapDbToJs({ ...dbUpdates, id }, storeName);

      // Optimistic update
      store = store.map(item => item.id === id ? jsUpdatedItem : item);
      api.notify();
